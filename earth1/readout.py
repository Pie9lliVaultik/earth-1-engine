...
# ── shape from data, not from physics (2026-08-18) ──

POL_ORDER = ["far_left", "left", "lean_left", "centrist",
             "lean_right", "right", "far_right"]


def cohort_shape(cohort_targets: dict, axis: str = "pol") -> dict | None:
    """Report the SHAPE as measured, not averaged away.

    Measured 2026-08-18: real within-cell densities are bimodal (61.5%
    extreme mass) while the engine's sigmoid readout produces 3.6%. The
    fix is not new physics — it is refusing to average. A GSS seed
    already carries, per political cohort:
        far_left 0.85 · centrist 0.60 · far_right 0.25
    which IS the bimodality. Reporting the cohort profile directly
    gives shape that came from real people.

    Returns the ordered profile, the spread, the polarization gap
    (extreme minus extreme) and the valley depth (how far the centre
    sits below the midpoint of the two poles) — a positive valley is
    the signature of two camps rather than a gradient.
    """
    keys = [k for k in cohort_targets if k.startswith(f"{axis}_")]
    if len(keys) < 3:
        return None
    prof = {k.replace(f"{axis}_", ""): float(cohort_targets[k])
            for k in keys}
    ordered = [(g, prof[g]) for g in POL_ORDER if g in prof]
    if len(ordered) < 3:
        ordered = sorted(prof.items())
    vals = [v for _, v in ordered]
    lo, hi = vals[0], vals[-1]
    mid = vals[len(vals) // 2]
    return {
        "axis": axis,
        "profile": [{"cohort": g, "share": round(v, 4)} for g, v in ordered],
        "spread": round(max(vals) - min(vals), 4),
        "polarization_gap": round(abs(hi - lo), 4),
        "valley_depth": round(((lo + hi) / 2.0) - mid, 4),
        "is_bimodal": bool(abs(hi - lo) > 0.30 and ((lo + hi) / 2.0) - mid > -0.05),
        "source": "measured cohort targets — NOT generated by the engine",
    }
