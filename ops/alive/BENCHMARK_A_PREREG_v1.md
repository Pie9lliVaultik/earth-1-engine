...
| (iv) held-out question generalization | weights via `calibrate.calibrate_from_neighbors` (cosine over force vectors of calibrated items) → readout on held-out countries | **semantic-neighbour** (country shares of the most text-similar GOQA item; cosine over `earth1.embedder.embed` sentence embeddings — AMENDMENT 2026-08-23, recorded before any Earth-1 readout: the registration text said hashed TF-IDF, but the repo embedder loads the sentence-embedding model when present and did so on prime; the executed baseline is therefore the STRONGER semantic neighbour, which only makes the gate harder); **LLM** = `claude-haiku-4-5-20251001`, temperature 0, frozen prompt (Appendix A) asking per-country yes-shares — **AWAITING AUTHORIZATION** (spends money; stop condition); if authorized it is scored by the frozen scorer on the same set and its answers cannot depend on Earth-1 output | beat the LLM/semantic-neighbour baseline (lower MAE, paired CI excluding 0) |
